...
from CLTreeModules_IterativeCompatible import Data, DataSetSummary, CLNode, CLTree
from CLTree_Utility import doThisPathExist, createPath, getRandomStringasId, clearTheDirectory, os
...
class PruneTreeByConqueringNodes(object):
    ...
    def prune(self, min_y, gradientTolerance=0.1, searchMode=True):
        ...
        'min_y' = {}, 'result' = \n{}".format(min_y, result2['data-points']))

            clearTheDirectory(self.tmpFilesPath)

            return result2

        else:
            logger.info(
                "Searching for 'min_y'. Current 'min_y' = {}, 'result' = \n{}".format(min_y, result['data-points']))
    ...
    def initializeRepresentationVectors(self, root):
        ...
        clTreeDataset = self.cltree.getDataset()
        assert clTreeDataset is not None, "No Dataset is attached with the CLTree at Tree Level"
        columns = clTreeDataset.instance_view.shape[1]  # Highest Column
        allColPos = clTreeDataset.attr_idx  # All the attributes used to create the Tree
    ...
    def pruningTreeByMergingRepresentationVectors(self, originalRepresentationVectors, min_y):
        ...
        for key in snapshot.keys():
            ...
            result = self.clusterStatistics(originalRepresentationVectors=copy.deepcopy(representationVectors),
                                            finalRepresentationVectors=copy.deepcopy(
                                                scenario_FinalRepresentationVectors),
                                            finalRepresentationVectorsToCluster=copy.deepcopy(
                                                tmp_scenario_finalRVToCluster),
        ...
        if node.includedInCluster:
            ...
            if lc and rc:
                ...
                if lcId == rcId:
                    ...
                    node.dataset.calculate_limits()
                    nodeDataSetPhysicalAddress = self.tmpFilesPath + node.getDatasetId() + ".pickle"
                    writePickleFile(path=nodeDataSetPhysicalAddress, data=node.dataset)
                    node.modifyUnderlyingDataSet(node.getDatasetId(), nodeDataSetPhysicalAddress)
                    return True
                else:
                    return False
            else:
    ...
    def clusterStatistics(self, originalRepresentationVectors, finalRepresentationVectors,
        ...
        if not self.useSilhouette:
            ...
            interClusterDistance, intraClusterDistance = self._calcClusterStatistics(
                allCombosOfFinalRepresentationVectors,
...
def pruneByGridSearch(cltree, min_y, prefixString, gradientTolerance, conquerData, divideData, useSilhouette,
    ...
    assert isinstance(cltree, CLTree) and isinstance(cltree.getRoot(), CLNode), logger.error( \
        "ERROR! Object Type mismatch. This function can operate only on CLTree and CLNode")
    assert isinstance(conquerData, pd.DataFrame), logger.error("ERROR! Object Type mismatch for 'conquerData'."
                                                               " Pandas.DataFrame is expected!")
    root = cltree.getRoot()
    ...
    conquerData.rename(columns=newColumnNames, inplace=True)
    if 'rownoasid' in newColumnNames.values():
        conquerData.set_index('rownoasid', drop=True, inplace=True)

    p = PruneTreeByConqueringNodes(prefixString, cltree, conquerData, useSilhouette, tmpFilesPath)
    ...
    if result is None:
        ...
        assert isinstance(divideData, pd.DataFrame), logger.error("ERROR! Object Type mismatch for 'divideData'."
                                                                  " Pandas.DataFrame is expected!")
        newColumnNames = dict()
        for col in divideData.columns:
            newColumnNames[col] = col.strip().lower()
        divideData.rename(columns=newColumnNames, inplace=True)
        if 'rownoasid' in newColumnNames.values():
            divideData.set_index('rownoasid', drop=True, inplace=True)

        p = PruneTreeByConqueringNodes(prefixString, cltree, divideData, useSilhouette, tmpFilesPath)
        p.resetAllPruneNodes(node=root)
