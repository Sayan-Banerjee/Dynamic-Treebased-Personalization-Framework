...
class DatasetSplitter:
    ...
    def _splitNrVirtualPoints(self, dataset, attribute, value, in_set, out_set):
        ...
        in_set.nr_virtual_points = max(int(abs(dataset.nr_virtual_points*((value-minV+1)/(maxV-minV+1)))), 1)
...
class BuildTree_Iterative(object):
    def __init__(self, min_split, tmpFilesPath='./'):
    ...
    def build(self, dataset):
        self._build_tree_iterative(dataset)
        return self.root

    def _build_tree_iterative(self, dataset):
        stack = list()
        parent = None
        while True:
            tmpRunningId = getRandomStringasId()
            tmpStoringPath = self.tmpFilesPath + "CLTree/" + str(tmpRunningId) + "/"
        ...
        datasetId = getRandomStringasId(self.maxNodeId)
        datasetPhysicalAddress = self.datasetDirectory + str(datasetId) + ".pickle"

        dataset.calculate_limits()
        writePickleFile(datasetPhysicalAddress, dataset)
        depth = 0
        ...
        while True:
            dataset = readPicklefile(path=datasetPhysicalAddress)
            ...
            if bestCut is not None:
                ...
                if lhs_dataset.getLength() > self.min_split:
                    datasetPhysicalAddress = L_datasetPhysicalAddress
                    parent = dt_node
...
class CLNode(object):
    ...
    def modifyUnderlyingDataSet(self, datasetId, datasetPhysicalAddress):
        if doThisPathExist(datasetPhysicalAddress):
            try:
...
class CLTree:
    ...
    def buildTree(self):
        b = BuildTree_Iterative(self.min_split)
        self.root = b.build(self.dataset)

        self.datasetDirectory = b.datasetDirectory
        self.storageDirectory = b.storageDirectory
