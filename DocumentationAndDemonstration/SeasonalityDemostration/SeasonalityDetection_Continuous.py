...
class GraphNode:
    ...
    def setRepresentationVector(self, vector):
        try:
            self.vector = _getUnitVector(vector)
        except Exception:
            self.vector = vector
...
class Graph:
    ...
    def findClusters(self, limitingCos, minMember=32, maxAcceptableClusters=5):
        ...
        while True:
            ...
            if maxCosSim < cosVal_lowerLimit and len(workingNodes)<=maxAcceptableClusters: # changes made here
                ...
                if len(notQualifiedMembers) == 0:
                    ...
                    if len(notQualifiedMembers) >= 2:
                        lookAndMergePairs = []
                        allNotQualifieldCombinations =                        sum([list(map(list, combinations(list(notQualifiedMembers.keys()), i)))                                                                     for i in range(2,3)], [])
                        ...
                        if len(lookAndMergePairs)>0:
                            recordKeeping = {nodeID_ij:cosSim for nodeID_ij, cosSim in recordKeeping.items()                                             if nodeID_ij in lookAndMergePairs}
            ...
            allConnectedNodes = workingNodes[maxCosSimGrp[0]].getConnected() +                                            workingNodes[maxCosSimGrp[1]].getConnected()
            ...
            toBeRemovedConnectionNodes = workingNodes[maxCosSimGrp[0]].getConnected() +                                            workingNodes[maxCosSimGrp[1]].getConnected()
            toBeRemovedConnectionNodes = list(set([node for node in toBeRemovedConnectionNodes if node.getID() in maxCosSimGrp]))
        ...
        for c in workingNodes.keys():
            ...
            val = workingNodes[c].originalNodeIds if workingNodes[c].originalNodeIds is not None else workingNodes[c].getID()
            clusters[name] = val
            i+=1
...
def ClusterMonths_Continuous(ds_EachRowADataPoint,
    ...
    for cos in cosDegrees:
        for maxAllowedClusters in acceptableNumberOfClusters:
            ...
            graph = buildTheGraph(ds_EachRowADataPoint, instanceCountByMonth, 'circular')
