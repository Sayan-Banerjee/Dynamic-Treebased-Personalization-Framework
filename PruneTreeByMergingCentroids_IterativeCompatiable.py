...
class PruneTreeByMergingCentroids(object):
    def __init__(self, prefixString, cltree, balancedPrune=False, useSilhouette=False, tmpFilesPath='./'):
        self.cltree = cltree
    ...
    def _calcInitialCentroids(self, preOrderedListofLeaves, attributes):
        ...
        rootDataset = copy.deepcopy(self.cltree.getRoot().getDatasetSummary())  # Make a copy of the original data set
        for i in preOrderedListofLeaves:
            node = self.cltree.nodes[i]
            ...
            nodeDataset = copy.deepcopy(node.getDatasetSummary())
    ...
    def initializeCentroids(self, root):
        ...
        attributes = root.getDatasetSummary().getAttributes()
    ...
    def pruningTreeByMergingRepresentationVectors(self, originalRepresentationVectors, min_y):
        ...
        for key in snapshot.keys():
            ...
            if not self.useSilhouette:
                logger.info("Min_Y: {}, Scenario: {},  #Clusters: {}, argminMetric: {}".
                            format(min_y, scenario.values(), key, result['argminMetric']))
    ...
    def clusterStatistics(self, originalRepresentationVectors, finalRepresentationVectors,
        ...
        if not self.useSilhouette:
            ...
            varPurity = np.sqrt(
                var) ** purity  # We want a balanced cluster at the same time a homogeneous cluster if balancedPrune
...
def pruneByGridSearch_Centroid(cltree, min_y, data_length, prefixString, balancedPrune, useSilhouette,
    ...
    p = PruneTreeByMergingCentroids(prefixString, cltree, balancedPrune, useSilhouette, tmpFilesPath)
    p.resetAllPruneNodes(node=root)
    result = p.prune(min_y=minLowerLimit, searchMode=False)
    if result is None:
