...
class ClusterTreeNode:
    ...
    def _setOperationCode(self, value):
        if isinstance(value, list) or isinstance(value, tuple):
            ...
            self.operationCode = 1  # Operation code stand for <= and >
...
class ClusterTree:
    ...
    def getClusterID(self, info):
        ...
        if len(info) > 0:
            ...
            logger.warning('getClusterID() has received empty key-value pair as parameter.'
                           ' It will assume pre-computed default values at each level to produce customer category!')
        ...
        while clusterID is None and curNode is not None:
            ...
            if opcode == 1:
                if isinstance(nodeVal, str):
                    logger.error("Unexpected data for Attribute: {}, Passed Value: {}, Numerical data expected!".
                                 format(att, nodeVal))
