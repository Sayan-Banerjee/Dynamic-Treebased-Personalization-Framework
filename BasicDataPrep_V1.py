...
def getData(sourcePath, categoricalAttributes, keyString="arrival", conquerDataColumns=None, tolerance=0.90,
            delimiter=';'):
    ...
    if conquerDataColumns is not None:
        ...
        assert len(
            set_cqr_cols - set_df_cols) == 0, "Some of the columns mentioned in 'conquerDataColumns'," \
                                              " do NOT exist in the provided data!"
    if conquerDataColumns is not None and categoricalAttributes is not None:
        assert len(set_cqr_cols.intersection(
            set(categoricalAttributes))) == 0, "Current version only supports numerical type conquer data!"
    if conquerDataColumns is not None:
        for col in conquerDataColumns:
            for ii in range(len(df)):
                ...
                if isinstance(val, str) and not val.replace('.', '', 1).lstrip('-').isdigit():
                    logger.error(
                        "Current version only supports numerical type conquer data! Error occurred for value: {}"
                        " at column: {} at row: {}".format(val, col, ii))
                    raise ValueError(
                        "Current version only supports numerical type conquer data! Error occurred for value: {}"
                        " at column: {} at row: {}".format(val, col, ii))
